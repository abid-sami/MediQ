...
import { LabStaffProfile } from "@/data/lab-staff-data";

interface LabStaffProfileModuleProps {
  profile: LabStaffProfile;
  onUpdateProfile: (updated: LabStaffProfile) => void;
}

export function LabStaffProfileModule({
  profile,
  onUpdateProfile,
}: LabStaffProfileModuleProps) {
  const [formData, setFormData] = useState<LabStaffProfile & { age?: number; gender?: string }>(profile);
  ...
  const handleProfileSave = (e: React.FormEvent) => {
    e.preventDefault();
    onUpdateProfile(formData as LabStaffProfile);
    toast.success("Pathologist Credentials Saved", {
      description: "Updated credentials across MediQ Pathology Network",
    });
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs opacity-80 mt-0.5">
                Age: {formData.age || 31} yrs | Gender: {formData.gender || "Male"} | {formData.department}
              </p>
            </div>
          </div>
        </div>
        ...
        <TabsContent value="details">
          <form onSubmit={handleProfileSave} className="bg-card border border-border rounded-2xl p-6 space-y-6">
            <h3 className="text-base font-bold flex items-center gap-2 border-b border-border pb-3">
              <Sparkles className="h-4 w-4 text-primary" /> Clinical Pathologist Info & Station Details
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label className="text-xs font-bold text-muted-foreground">Full Name & Degrees</Label>
                <Input
                  value={formData.name}
              ...
              <div>
                <Label className="text-xs font-bold text-muted-foreground">License Registration No</Label>
                <Input
              ...
              <div>
                <Label className="text-xs font-bold text-muted-foreground">Laboratory Department</Label>
                <Input
              ...
              <div>
                <Label className="text-xs font-bold text-muted-foreground">Hospital Campus</Label>
                <Input
