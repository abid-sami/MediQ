...
import { BloodBankStaffProfile } from "@/data/blood-bank-data";

interface BloodBankProfileModuleProps {
  profile: BloodBankStaffProfile;
  onUpdateProfile: (updated: BloodBankStaffProfile) => void;
}

export function BloodBankProfileModule({
  profile,
  onUpdateProfile,
}: BloodBankProfileModuleProps) {
  const [formData, setFormData] = useState<BloodBankStaffProfile & { age?: number; gender?: string }>(profile);
  ...
  const handleProfileSave = (e: React.FormEvent) => {
    e.preventDefault();
    onUpdateProfile(formData as BloodBankStaffProfile);
    toast.success("Blood Bank Staff Credentials Saved", {
      description: "Updated administrative credentials across MediQ Repository System",
    });
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs opacity-80 mt-0.5">
                Age: {formData.age || 30} yrs | Gender: {formData.gender || "Male"} | {formData.facility}
              </p>
            </div>
          </div>
        </div>
        ...
        <TabsContent value="details">
          <form onSubmit={handleProfileSave} className="bg-card border border-border rounded-2xl p-6 space-y-6">
            <h3 className="text-base font-bold flex items-center gap-2 border-b border-border pb-3">
              <Sparkles className="h-4 w-4 text-primary" /> Staff Profile & Repository Station Details
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <Label className="text-xs font-bold text-muted-foreground">Facility Station</Label>
                <Input
